import { c as create_ssr_component, a as subscribe, d as add_attribute, e as each, f as escape, v as validate_component } from "../../chunks/index.js";
import { p as prefetchRoutes } from "../../chunks/navigation.js";
import { d as derived, r as readable } from "../../chunks/index2.js";
...
const css = {
  code: "ion-item.svelte-1yfkf6t{cursor:pointer}.menuhide.svelte-1yfkf6t{display:none}",
  map: null
};
const Menu = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  let menuItems = componentList.map((componentName) => {
    const url = componentName !== "Tabs" ? `/components/${componentName}` : `/components/tabs/[tab]`;
    return {
      url,
      label: componentName,
  ...
  return `<ion-menu${add_attribute("side", side, 0)} content-id="${"main"}" menu-id="${"mainmenu"}" class="${["svelte-1yfkf6t", hideMenu ? "menuhide" : ""].join(" ").trim()}">${menuItems.length > 0 ? `<ion-header><ion-toolbar translucent="${"true"}"><ion-title>Menu</ion-title></ion-toolbar></ion-header>
		<ion-content><ion-list>${each(menuItems, (menuItem, i) => {
    return `<ion-item class="${"svelte-1yfkf6t"}"><ion-icon${add_attribute("icon", menuItem.icon, 0)} slot="${"start"}"${add_attribute("color", getRandomColor(), 0)}></ion-icon>
						<ion-label>${escape(menuItem.label)}</ion-label>
					</ion-item>`;
  })}

				<ion-item class="${"svelte-1yfkf6t"}"></ion-item>
				${iosInstall ? `<ion-item class="${"svelte-1yfkf6t"}"><ion-icon${add_attribute("icon", allIonicIcons["download"], 0)} slot="${"start"}"></ion-icon>
						<ion-label>Install this app as PWA</ion-label></ion-item>
					<ion-item class="${"svelte-1yfkf6t"}"></ion-item>` : ``}
				${$canInstall ? `<ion-item class="${"svelte-1yfkf6t"}"><ion-icon${add_attribute("icon", allIonicIcons["download"], 0)} slot="${"start"}"></ion-icon>
						<ion-label>Install this app as PWA</ion-label></ion-item>
					<ion-item class="${"svelte-1yfkf6t"}"></ion-item>` : ``}

				<ion-item class="${"svelte-1yfkf6t"}"><ion-icon${add_attribute("icon", allIonicIcons["star"], 0)} slot="${"start"}"></ion-icon>
					<ion-label>Go to GitHub for this app</ion-label></ion-item>
				<ion-item class="${"svelte-1yfkf6t"}"><ion-icon${add_attribute("icon", allIonicIcons["star"], 0)} slot="${"start"}"></ion-icon>
					<ion-label>Go to Ionic Forum</ion-label></ion-item></ion-list></ion-content>` : ``}
</ion-menu>`;
});
const variables = "";
